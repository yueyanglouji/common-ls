...
public class Async {
    ...
    static class ExecRunnable<T> implements Runnable {
        ...
        private final org.apache.hc.client5.http.fluent.Request request;
        private final Executor executor;
        private final HttpClientResponseHandler<T> handler;

        ExecRunnable(
                final BasicFuture<T> future,
                final org.apache.hc.client5.http.fluent.Request request,
        ...
        public void run() {
            try {
                final org.apache.hc.client5.http.fluent.Response response = this.executor.execute(this.request);
    ...
    public <T> Future<T> execute(
            final org.apache.hc.client5.http.fluent.Request request, final HttpClientResponseHandler<T> handler, final FutureCallback<T> callback) {
    ...
    public <T> Future<T> execute(final org.apache.hc.client5.http.fluent.Request request, final HttpClientResponseHandler<T> handler) {
        return execute(request, handler, null);
    }

    public Future<Content> execute(final org.apache.hc.client5.http.fluent.Request request, final FutureCallback<Content> callback) {
        return execute(request, new ContentResponseHandler(), callback);
    }

    public Future<Content> execute(final org.apache.hc.client5.http.fluent.Request request) {
